...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Inicializar botão de formulário
    initializeFormularioButton();
});

// ===== CONTROLE DO BOTÃO DE FORMULÁRIO =====

function initializeFormularioButton() {
    const formularioBtn = document.getElementById('formularioBtn');
    const createSection = document.querySelector('.create-section');
    
    if (!formularioBtn || !createSection) {
        console.warn('[WARNING] Botão de formulário ou seção não encontrados');
        return;
    }
    
    // Estado inicial - formulário visível
    ...
    formularioBtn.addEventListener('click', function() {
        ...
        if (isFormVisible) {
            ...
            formularioBtn.classList.remove('active');
            
            // Mudar ícone para olho aberto
            const svg = formularioBtn.querySelector('svg');
            svg.innerHTML = `
            ...
            formularioBtn.classList.add('active');
        ...
        console.log(`[INFO] Formulário ${isFormVisible ? 'mostrado' : 'escondido'}`);
    });
    
    console.log('[SUCCESS] Botão de formulário inicializado');
}